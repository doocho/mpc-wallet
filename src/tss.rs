...
pub trait Threshold {
	...
	fn sign_digest(&self, digest32: &[u8], client_share: &FieldBytes, server_share: &FieldBytes) -> anyhow::Result<Signature>;
}
...
impl Threshold for LocalAdditiveTss {
	fn keygen(&mut self) -> anyhow::Result<(Vec<u8>, FieldBytes, FieldBytes)> {
		...
		let x = if bool::from(x_ct.is_some()) { x_ct.unwrap() } else { anyhow::bail!("invalid scalar repr") };
	...
	fn sign_digest(&self, digest32: &[u8], client_share: &FieldBytes, server_share: &FieldBytes) -> anyhow::Result<Signature> {
		if digest32.len() != 32 { anyhow::bail!("digest must be 32 bytes"); }
...
impl Threshold for Gg18Tss {
	fn keygen(&mut self) -> anyhow::Result<(Vec<u8>, FieldBytes, FieldBytes)> {
		unimplemented!("Integrate ZenGo multi-party-ecdsa GG18 here")
	}
	fn sign_digest(&self, _digest32: &[u8], _client_share: &FieldBytes, _server_share: &FieldBytes) -> anyhow::Result<Signature> {
		unimplemented!("Integrate ZenGo multi-party-ecdsa GG18 here")
	}
}
